...
const generateContent = async () => {
  ...
  try {
    setLoading(true);
    console.log('Sending request to /api/blog/generate with prompt:', title); // Detailed log
    const { data } = await axios.post('/api/blog/generate', { prompt: title });
    console.log('Received response from /api/blog/generate:', data); // Detailed log
    if (data.success) {
      const sanitizedContent = DOMPurify.sanitize(await parse(data.content)); // Await and sanitize
      quillRef.current.root.innerHTML = sanitizedContent;
    } else {
      toast.error(data.message || 'Failed to generate content');
    }
  } catch (error) {
    console.error('Generate content error:', error.message, error.response?.data); // Detailed error log
